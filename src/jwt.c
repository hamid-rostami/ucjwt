 ...
 */
unsigned char *
jwt_hs256_gen(char *key, size_t key_len, char *msg, size_t msg_len) {
...
int jwt_encode(char *payload,
  ...
  if (payload == NULL || payload_size == 0 || key == NULL || key_size == 0 ||
      token == NULL || token_size == 0)
    return -1;

  payload_b64 = base64_encode((const unsigned char *)payload, payload_size,
                              &payload_b64_len);
...
JWTDecode jwt_decode(char *token,
  ...
  payload = base64_decode((const unsigned char *)payload_b64, payload_b64_len,
                          &payload_len);
  ...
  sign = jwt_hs256_gen(key, key_size, token,
                       header_b64_len + payload_b64_len +
                           1); // +1 for separating dot
