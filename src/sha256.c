 ...
 */
uint32_t rotr32(uint32_t x, uint8_t n) { return ((x >> n) | (x << (32 - n))); }
...
void sha256_lastBlock(sha256_ctx_t *state, const void *block, uint16_t length) {
  ...
  length = (length >> 3) +
           1; /* from now on length contains the number of BYTES in lb*/
  /* pad with zeros */
...
void sha256(
    sha256_hash_t *dest,
    const void *msg,
    uint32_t length) { /* length could be choosen longer but this is for µC */
  sha256_ctx_t s;
  sha256_init(&s);
  while (length >= SHA256_BLOCK_BITS) {
